--duplicates elimination in predicted exp
...
where s.item_code = e.item_code and e.comp_str_id = 5651 and to_date(pred_date,'yyyy-MM-dd') = to_date('07/25/2013','MM/dd/yyyy')
)
where s.tod_id = 4  and s.schedule_id = 6367 and s.dow_id = 5

--0233
update summary_daily_mov s set s.sixty_four_exp = 
(select pred from predicted_exp e 
where s.item_code = e.item_code and e.comp_str_id = 5673 and to_date(pred_date,'yyyy-MM-dd') = to_date('07/25/2013','MM/dd/yyyy')
)
where  s.tod_id =4 and s.schedule_id = 6369 and s.dow_id = 5
...
where comp_str_id = 5651 and  to_date(pred_date,'yyyy-MM-dd') = to_date('07/25/2013','MM/dd/yyyy'))
...
where  s.tod_id =4 and s.schedule_id = 6369 and s.dow_id = 5
and item_code in(select item_code from predicted_exp_focus e 
where comp_str_id = 5673 and to_date(pred_date,'yyyy-MM-dd') = to_date('07/25/2013','MM/dd/yyyy'))
