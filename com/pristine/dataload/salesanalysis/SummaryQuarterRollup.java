/*
 * Title: TOPS Summary Quarter RollUp
 *
 *******************************************************************************
 ...
 * Version 0.7  23/01/2013  John Britto     Log re-factored
 *******************************************************************************
 */
...
public class SummaryQuarterRollup {

	static Logger logger = Logger.getLogger("SummaryQuarterRollup");

	// global variables
	private Date _processDate = null;
	private Date _startDate = null; // hold the quarter start date
	...
	int _quarterCalendarId = 0; // hold the quarter calendar id for given quarter
	 ...
	 * ****************************************************************
	 */
	public SummaryQuarterRollup(Date processDate, String districtId,
		...
		if (districtId != null) {
			...
			logger.info("Summary Quarter Roll up For District:" + _locationId);
		}
		...
		if (regionId != null) {
			...
			logger.info("Summary Quarter Roll up For Region:"+ _locationId);
		}
		...
		if (divisionId != null) {
			...
			logger.info("Summary Quarter Roll up For Division:" + _locationId);
		}
		...
		if (chainId != null) {
			...
			_chainId = chainId;
			_locationId = chainId;
			logger.info("Summary Quarter Rollup For Chain:" + _locationId);
		}
	 ...
	 * not specified then process for past quarter
	 * ****************************************************************
	 */
	...
	public static void main(String[] args) throws GeneralException {
		...
		PropertyConfigurator
				.configure("log4j-summary-quarter-rollup.properties");

		logger.info("*** Summary periodly Rollup Process Begins ***");
		logCommand(args);
		...
		SummaryQuarterRollup summaryperiodRollup = null;
		try {
			...
			summaryperiodRollup = new SummaryQuarterRollup(processDate,
					districtId, regionId, divisionId, chainId, reCalculation,
					repairFlag);

			// Process Roll up Starts
			summaryperiodRollup.processQuarterRollupData();

		} catch (Exception exe) {
			logger.error("Quarter Rollup Error....." + exe);
			throw new GeneralException("Quarter Rollup Error....." + exe);
		} finally {
			logger.info("*** Summary periodly Rollup Process Ends ***");
	...
	private void processQuarterRollupData() throws GeneralException {
		...
		// hold the last year Sales Period Roll up Id id
		// key location_level_id and location_id
		// value Sales Period Roll up Id
				...
				if (repairCalendarDates != null) {
					...
					List<RetailCalendarDTO> repairWeekCalendarList = objCalendarDao
							.getRepairCalendarList(_conn,
									...
									_processDate, Constants.CALENDAR_QUARTER);
					...
					for (int rWC = 0; rWC < repairWeekCalendarList.size(); rWC++) {
						...
						if (objCalendarDao.checkWeekStartDate(_conn,
							...
							objWRDao.updateSalesRollup(_conn,
									...
									lastUpdatedCalendarId, _quarterCalendarId,
						...
						getquarterRollupAggregation(dailyCalendarList,
								objCalendarDao, objSalesDao, objCtdDao,
								lastSummaryList);
					}
				}
				else {
					processQuarterRollup(objCalendarDto, objCalendarDao,
	...
	private void processQuarterRollup(RetailCalendarDTO objCalendarDto,
		...
		try {
			...
			if (_reCalculation != "" && _reCalculation.equalsIgnoreCase("YES")) {
				...
				// delete the previous aggregation in Sales table
				objSalesDao.deletePreviousSalesAggregation(_conn,
						_quarterCalendarId, Integer.valueOf(_locationId),
						_locationLevelId, "SALES_AGGR_ROLLUP");
			...
			lastSummaryList = objSalesDao.getLastYearSalesAggrList(_conn,
					...
					"SALES_AGGR_ROLLUP", "SALES_AGGR_ROLLUP_ID");
			// call the Period aggregation process
			getquarterRollupAggregation(dailyCalendarList, objCalendarDao,
					objSalesDao, objCtdDao, lastSummaryList);
		} catch (NumberFormatException e) {
			 throw new GeneralException(" Error While Aggregating Quarter Rollup....." , e);
	...
	private void getquarterRollupAggregation(
			List<RetailCalendarDTO> dailyCalendarList,
		...
		try {
			...
			if (LastAggrCalendarId != 0) {
				...
				quarterMap = objSalesRollupDao.rollupAggregation(_conn,
						_quarterCalendarId, _locationId, _locationLevelId,
						"SALES_AGGR_ROLLUP");
			...
			for (int cL = 0; cL < dailyCalendarList.size(); cL++) {
				...
				PristineDBUtil.commitTransaction(_conn,
						"Quarter Rollup Comitted");
			}

		} catch (Exception exe) {
			PristineDBUtil.rollbackTransaction(_conn,
					"Quarter Rollup Rollbacked");
			logger.error(" Quarter Rollup Error...... " + exe);
			throw new GeneralException(" Quarter Rollup Error...... " + exe);
		}
	...
	private static void logCommand(String[] args) {
		StringBuffer sb = new StringBuffer("Command: Summary Quarter  Rollup ");
