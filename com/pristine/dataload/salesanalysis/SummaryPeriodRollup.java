/*
 * Title: TOPS Summary period RollUp
 *
 *******************************************************************************
 ...
 *------------------------------------------------------------------------------
 * Version 0.1	10/04/2012	Dinesh Kumar	Initial Version 
...
public class SummaryPeriodRollup {

	static Logger logger = Logger.getLogger("SummaryPeriodRollup");

	// global variables
	private Date _processDate   = null;
	private Date _startDate 	= null;         // hold the period start date
	...
	int _periodCalendarId 		= 0;            // hold the period calendar id for given period
	 ...
	 * ****************************************************************
	 */
	public SummaryPeriodRollup(Date processDate, String districtId,
		...
		if (districtId != null) {
			...
			logger.info("Summary Period Rollup For District:"	+ _locationId);
		}
		...
		if (regionId != null) {
			...
			logger.info("Summary Period Rollup For Region:"+ _locationId);
		}
		...
		if (divisionId != null) {
			...
			logger.info("Summary Periodly Rollup For Division:"	+ _locationId);
		}
		...
		if (chainId != null) {
			...
			_chainId = chainId;
			_locationId = chainId;
			logger.info("Summary Period Rollup For Chain:" + _locationId);
		}
	 ...
	 * Arguemnt 2: Process Id
	 * If the StartDate and EndDate is not  specified then process for past period
	 * ****************************************************************
	 */
	...
	public static void main(String[] args) throws GeneralException {
		...
		PropertyConfigurator
				.configure("log4j-summary-period-rollup.properties");

		logger.info("*** Summary period Rollup Process Starts ***");
		logCommand(args);
		...
		SummaryPeriodRollup summaryperiodRollup = null;
		try {
			...
			summaryperiodRollup = new SummaryPeriodRollup(processDate,
					districtId, regionId, divisionId, chainId, reCalculation ,repairFlag);

			// Process Roll up Starts
			summaryperiodRollup.processperiodlyRollupData();

		} catch (Exception exe) {
			logger.error(" Period Rollup Process Error.... " + exe);
			throw new GeneralException(" Period Rollup Process Error.... " + exe);
		} finally {
			logger.info("*** Summary period Rollup Process Starts ***");			
			PristineDBUtil.close(summaryperiodRollup._conn);
		}
	}
	...
	private void processperiodlyRollupData() throws GeneralException {
		...
		// hold the last year Sales Period Rollup Id id
		// key location_level_id and location_id
		// value Sales Period Rollup Id
			...
			if (_repairFlag.equalsIgnoreCase("FULL")) {
				...
				if (repairCalendarDates != null) {
					...
					for (int rWC = 0; rWC < repairWeekCalendarList.size(); rWC++) {
						...
						_periodCalendarId = objWeekCalDto.getCalendarId();
						...
						if (objCalendarDao.checkWeekStartDate(_conn,
							...
							logger.debug("Repair start Last Calenar:"
							...
							objWRDao.updateSalesRollup(_conn,
									...
									lastUpdatedCalendarId, _periodCalendarId,
						...
						getperiodlyRollupAggregation(dailyCalendarList,
					...
					processPeriodRollup(objCalendarDto, objCalendarDao,
	...
	private void processPeriodRollup(RetailCalendarDTO objCalendarDto,
		...
		try {
			...
			if (_reCalculation != "" && _reCalculation.equalsIgnoreCase("YES")) {
				...
				// delete the previous aggregation in Sales table
				objSalesDao.deletePreviousSalesAggregation(_conn,
						_periodCalendarId, Integer.valueOf(_locationId),
						_locationLevelId, "SALES_AGGR_ROLLUP");
			...
			lastSummaryList = objSalesDao.getLastYearSalesAggrList(_conn,
					...
					"SALES_AGGR_ROLLUP", "SALES_AGGR_ROLLUP_ID");
			// call the Period aggregation process
			getperiodlyRollupAggregation(dailyCalendarList, objCalendarDao,
					objSalesDao, objCtdDao, lastSummaryList);
		} catch (NumberFormatException e) {
			throw new GeneralException(
					" Error While Processing Period Rollup .....", e);
	...
	/*
	 * ****************************************************************
	 * Aggregate the district , region , division , chain reocrds  
	 * Sales Quarter Process
	...
	private void getperiodlyRollupAggregation(
			List<RetailCalendarDTO> dailyCalendarList,
			RetailCalendarDAO objCalendarDao, SalesAggregationDao objSalesDao,
			SalesAggregationCtdDao objCtdDao, HashMap<String, Long> lastSummaryList) throws  GeneralException {
		logger.debug(" Get period Aggregation Records Starts ");
		...
		try {
			...
			if (LastAggrCalendarId != 0) {
				...
				periodMap = objSalesRollupDao.rollupAggregation(_conn,
						_periodCalendarId, _locationId, _locationLevelId,
						"SALES_AGGR_ROLLUP");
			...
			for (int cL = 0; cL < dailyCalendarList.size(); cL++) {
				...
				if (LastAggrCalendarId < calendarDto.getCalendarId()) {
					...
					HashMap<String, SummaryDataDTO> dailyMap = objSalesRollupDao
							.rollupAggregation(_conn, calendarDto.getCalendarId(),
									_locationId, _locationLevelId,
									"SALES_AGGR_DAILY_ROLLUP");
					...
					if (dailyMap.size() != 0) {
						...
						// call the businesslogic method to sum up the daily and periodly records
						productMap = objSalesBusiness
									.sumupAggregation(dailyMap, productMap, calendarDto.getCalendarId());

						// updated periodly records .
						objSalesRollupDao.updateSalesRollup(_conn, productMap,
									calendarDto.getCalendarId(),
									_periodCalendarId, "SALES_AGGR_ROLLUP" ,"ACTUAL");

							// Insert New periodly Records
						objSalesRollupDao.insertSalesRollup(_conn, dailyMap,
									calendarDto.getCalendarId(),
									_periodCalendarId, "SALES_AGGR_ROLLUP", lastSummaryList);
				...
				PristineDBUtil.commitTransaction(_conn, "Period Rollup Commited");
			}

		} catch (Exception exe) {
			PristineDBUtil.rollbackTransaction(_conn, "Period Rollup Rollbacked");
			logger.error(" Period Rollup Process Error.... " , exe);
			throw new GeneralException(" Period Rollup Process Error.... " , exe);
		}
    ...
    {
		StringBuffer sb = new StringBuffer("Command: Summary Period Rollup ");
