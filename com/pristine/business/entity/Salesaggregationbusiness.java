...
public class Salesaggregationbusiness {
	...
	public void calculateProductSummary(
		...
		logger.debug(" Iterate the daily summary records starts ");
		...
		HashMap<String, SummaryDataDTO> posDepartmentMap = new HashMap<String, SummaryDataDTO>();
		...
		int posGasDept = Constants.POS_GASDEPARTMENT;
		...
		for (int ii = 0; ii < movementList.size(); ii++) {
			...
			int posId = objMoveDto.getPosDepartment();
			...
			if (posId == posGasDept) {
				...
				// code added for get the gas total for pos department
				if (posDepartmentMap.containsKey(String.valueOf(posId))) {
					...
					aggregateDatadto(objMoveDto, posDepartmentMap,
							"ADDPROCESS", String.valueOf(posId), posLevelId,
							null, processDealCost);
				} else {
					...
					aggregateDatadto(objMoveDto, posDepartmentMap,
							"NEWPROCESS", String.valueOf(posId), posLevelId,
							null, processDealCost);
				}
			}
			...
			// end...............................................................
				...
				// Aggregate POS Department Data
				if (posDepartmentMap.containsKey(String.valueOf(posId))) {
					// Update POS aggregation data into Map
					aggregateDatadto(objMoveDto, posDepartmentMap,
							"ADDPROCESS", String.valueOf(posId), posLevelId,
							null, processDealCost);
				} else {
					
					// Add POS aggregation data into Map
					aggregateDatadto(objMoveDto, posDepartmentMap,
							"NEWPROCESS", String.valueOf(posId), posLevelId,
							null, processDealCost);
				}
		...
		// Subtract the coupon values from the POS revenue
		Object[] copArray = posDepartmentMap.values().toArray();

		for (int cop = 0; cop < copArray.length; cop++) {
			SummaryDataDTO objPosDto = (SummaryDataDTO) copArray[cop];

			double couponRevenue = 0.0;
			if (couponAggregationMap.containsKey(objPosDto.getProductId())) {
								
				couponRevenue = couponAggregationMap.get(objPosDto.getProductId());

				objPosDto.setRegularRevenue(objPosDto.getRegularRevenue()
						- couponRevenue);
				storeCouponRevenue += couponRevenue;
				couponAggregationMap.remove(objPosDto.getProductId());
				posDepartmentMap.put(objPosDto.getProductId(), objPosDto);
			}
		}
		
		// add the non pos coupon revenue's into pos map
		
		Iterator<Entry<String, Double>> it = couponAggregationMap.entrySet().iterator();
		
		while( it.hasNext()){
			
			Entry<String , Double>  pairs =  (Entry<String , Double>) it.next();
			
			SummaryDataDTO objcouponDto = new SummaryDataDTO();
			objcouponDto.setProductId(pairs.getKey());
			objcouponDto.setProductLevelId(posLevelId);
			objcouponDto.setRegularRevenue(0 - pairs.getValue());
		 
			posDepartmentMap.put(pairs.getKey(), objcouponDto);
			storeCouponRevenue += pairs.getValue();
		}
		
		// Temporary Code Added for promotion department
		SummaryDataDTO objcouponDto = new SummaryDataDTO();
		objcouponDto.setProductId(String.valueOf(Constants.POS_PROMODEPARTMENT));
		objcouponDto.setProductLevelId(posLevelId);
		objcouponDto.setRegularRevenue(100);
		posDepartmentMap.put(String.valueOf(Constants.POS_PROMODEPARTMENT), objcouponDto);
	    // Temp code ended.................
		...
		objSummarryDto.setRegularRevenue(totRegularRevenue - storeCouponRevenue);
		...
		logger.debug("Pos Department map size:" + posDepartmentMap.size());
