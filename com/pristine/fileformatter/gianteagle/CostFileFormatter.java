...
public class CostFileFormatter extends PristineFileParser {
	...
	private static boolean loadFutureWeekCost = true;

	public CostFileFormatter() {
		super();
	...
	public static void main(String[] args) {
		...
		CostFileFormatter priceFileFormatter = new CostFileFormatter();
		for (String arg : args) {
	...
	/**
	 * Gets all items from Itemlookup and creates Map with UPC as key
	 * 
						...
						for (ItemDTO itemDTO : itemList) {
							if (itemDTO.getRetailerItemCode() != null && !itemDTO.getRetailerItemCode().isEmpty()) {
								...
								pw.print(giantEagleCostDTO.getALLW_AMT() > 0 ? round(giantEagleCostDTO.getALLW_AMT(), 4) : Constants.EMPTY);// Allow
																																			// Amt
								pw.print("|");
								pw.print(giantEagleCostDTO.getALLW_AMT() > 0 ? giantEagleCostDTO.getLONG_TERM_REFLECT_FG() : Constants.NO);// Long term flag
								pw.print("|");
