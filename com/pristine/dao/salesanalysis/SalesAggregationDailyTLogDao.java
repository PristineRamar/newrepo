...
public class SalesAggregationDailyTLogDao {
	...
	static String temp_insert_query = "INSERT INTO SALES_AGGR_DAILY_MOVEMENT_TEMP(CALENDAR_ID,  COMP_STR_NO , PRODUCT_LEVEL_ID , PRODUCT_ID ,  ITEM_CODE ,  SALE_FLAG ,  UOM_ID ,  POS_DEPARTMENT , " + 
			...
			"SELECT <cal_id>,CS.COMP_STR_NO ,<least_level>, PR0.PRODUCT_ID as PRODUCT_ID0,  MV.ITEM_ID, MV.SALE_TYPE as FLAG, I.UOM_ID, MV.POS_DEPARTMENT_ID,  " + 
			"sum(MV.QUANTITY * I.ITEM_SIZE) as VOLUME, sum(MV.QUANTITY) as QUANTITY, sum(MV.WEIGHT) as WEIGHT_ACTUAL, count(case when MV.WEIGHT=0  then null else 1 end) as WEIGHT, sum( MV.regular_amt) as REVENUE,  " + 
			"sum(MV.net_amt) as GROSSREVENUE " + 
			"from TRANSACTION_LOG MV Left Join ITEM_LOOKUP I On MV.ITEM_ID=I.ITEM_CODE AND I.CATEGORY_ID NOT IN (<exclude_category>) " +
			"INNER JOIN COMPETITOR_STORE CS ON MV.STORE_ID = CS.COMP_STR_ID " +
			"LEFT OUTER JOIN PRODUCT_GROUP_RELATION PR0 ON MV.ITEM_ID = PR0.CHILD_PRODUCT_ID AND PR0.PRODUCT_LEVEL_ID=<least_level>  " + 
			"where  MV.CALENDAR_ID = <cal_id> and MV.POS_DEPARTMENT_ID <= 39 and CS.COMP_STR_NO in ( " + 
			...
			"group by CS.COMP_STR_NO, PR0.PRODUCT_ID,  MV.ITEM_ID, MV.SALE_TYPE, I.UOM_ID, MV.POS_DEPARTMENT_ID " +
			"order by PR0.PRODUCT_ID,  MV.ITEM_ID, MV.SALE_TYPE  ";
	...
	static String summary_daily_visitcount =  
			"SELECT CS.COMP_STR_ID,P0.PRODUCT_ID , COUNT(DISTINCT M.TRX_NO) as VISIT_COUNT from TRANSACTION_LOG M LEFT JOIN ITEM_LOOKUP I ON I.ITEM_CODE = M.ITEM_ID " + 
			" <product_hierarchy_query> "+ 
			" INNER JOIN COMPETITOR_STORE CS ON CS.COMP_STR_ID = M.STORE_ID "+
			" where  M.CALENDAR_ID =<cal_id> and M.POS_DEPARTMENT_ID <=" + Constants.POS_GASDEPARTMENT +" and ( CS.COMP_STR_NO ='<store_no>' OR CS.district_id = <district_no> ) " + 
	...
	static String pos_insert_query = "INSERT INTO SALES_AGGR_DAILY(SUMMARY_DAILY_ID,CALENDAR_ID,LOCATION_LEVEL_ID,LOCATION_ID,PRODUCT_LEVEL_ID,PRODUCT_ID,TOT_VISIT_CNT, " + 
			...
			"REG_MARGIN_PCT,SALE_MARGIN_PCT,LOYALTY_CARD_SAVING,LST_SUMMARY_DAILY_ID,SUMMARY_CTD_ID,TOT_MOVEMENT_VOL,REG_MOVEMENT_VOL,SALE_MOVEMENT_VOL,TOT_IGVOL_REVENUE, " + 
			"REG_IGVOL_REVENUE,SALE_IGVOL_REVENUE,STORE_TYPE) " + 
			"SELECT "+DBManager.getIdentityIncrement("sales_aggr_daily_seq") +",<cal_id>,"+Constants.STORE_LEVEL_ID+", CS.COMP_STR_ID,"+Constants.POSDEPARTMENT+", M.POS_DEPARTMENT_ID AS PRODUCT_ID, 0 VISIT_COUNT,  " + 
			"SUM(TOT_MOVEMENT) TOT_MOVEMENT,SUM(REG_MOVEMENT) REG_MOVEMENT,SUM(SALE_MOVEMENT) SALE_MOVEMENT, " + 
			"SUM(TOT_REVENUE) TOT_REVENUE,SUM(REVENUEREGULAR  ) REG_REVENUE,SUM(REVENUESALE ) SALE_REVENUE, " + 
			"0 AVG_ORDER_SIZE, " + 
			"SUM(TOT_MARGIN) TOT_MARGIN, SUM(REG_MARGIN) REG_MARGIN,SUM(SALE_MARGIN) SALE_MARGIN, " + 
			...
			"GROUP BY CS.COMP_STR_ID ,M.POS_DEPARTMENT_ID ORDER BY M.POS_DEPARTMENT_ID ";
	
	static String pos_visit_count = "SELECT CS.COMP_STR_ID, M.POS_DEPARTMENT_ID AS PRODUCT_ID , COUNT(DISTINCT M.TRX_NO) as VISIT_COUNT from TRANSACTION_LOG M LEFT JOIN ITEM_LOOKUP I ON I.ITEM_CODE = M.ITEM_ID " + 
			" INNER JOIN COMPETITOR_STORE CS ON CS.COMP_STR_ID = M.STORE_ID "+
			" where  M.CALENDAR_ID =<cal_id> AND M.ITEM_ID IN (<item_code_list>) and ( CS.COMP_STR_NO ='<store_no>' OR CS.district_id = <district_no> ) " + 
			"  and (I.CATEGORY_ID is null or I.CATEGORY_ID NOT IN (<exclude_category>)) " + 
			" group by CS.COMP_STR_ID, M.POS_DEPARTMENT_ID   " ;
	...
	static String store_summary_daily_insert_query = "INSERT INTO SALES_AGGR_DAILY(SUMMARY_DAILY_ID,CALENDAR_ID,LOCATION_LEVEL_ID,LOCATION_ID,PRODUCT_LEVEL_ID,PRODUCT_ID,TOT_VISIT_CNT, " + 
			...
			"INNER JOIN COMPETITOR_STORE CS ON M.COMP_STR_NO = CS.COMP_STR_NO " +
			"LEFT JOIN ( " + 
			"SELECT M.STORE_ID , COUNT(DISTINCT M.TRX_NO) as VISIT_COUNT from TRANSACTION_LOG M LEFT JOIN ITEM_LOOKUP I ON I.ITEM_CODE = M.ITEM_ID " + 
			"where  M.CALENDAR_ID =<cal_id> and M.POS_DEPARTMENT_ID <=" + Constants.POS_GASDEPARTMENT +" and M.STORE_ID IN ( " + 
			...
			"group by M.STORE_ID  " + 
			") TBL_VISIT_COUNT on TBL_VISIT_COUNT.STORE_ID = CS.COMP_STR_ID " + 
	...
	public void moveTempToSalesAggregation(Connection _Conn, ArrayList<ProductGroupDTO> lstPGDTO, int calendarId, String compStrNo, 
		...
		try {
			...
			if(productGroupLength > 1 ){
				...
				//Only Item_Id is present in Transaction_Log
				product_hierarchy_query = product_hierarchy_query.replaceAll("M.ITEM_CODE","M.ITEM_ID");
				tempSql = tempSql.replaceAll("<product_hierarchy_query>", product_hierarchy_query );
				tempSql = tempSql.replaceAll("<parent_level>", parentLevelID+"");
				tempSql = tempSql.replaceAll("<exclude_category>", excludeCategory+"");
	...
	public void markProcessCompletionForMovementDaily(Connection _Conn) throws GeneralException {
		StringBuffer sql = new StringBuffer();
		sql.append(" select calendar_id,CS.COMP_STR_ID from SALES_AGGR_DAILY_MOVEMENT_TEMP M INNER JOIN COMPETITOR_STORE CS ON M.COMP_STR_NO = CS.COMP_STR_NO  group by calendar_id,CS.COMP_STR_ID ");
		...
		try{
			...
			PreparedStatement psmt = _Conn.prepareStatement("update TRANSACTION_LOG set PROCESSED_FLAG='Y' WHERE CALENDAR_ID = ? AND STORE_ID = ? ");
